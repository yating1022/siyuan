<template>
  <div
    class="max-w-3xl px-4 pb-8 mx-auto mt-10 mb-20 sm:px-6 xl:max-w-5xl xl:px-0 dark:divide-slate-200/20"
  >
    <!-- 标题 -->
...
// hideHeader: 当页面已有自己的 hero(如首页)时,隐藏本组件自带的标题
const { hideHeader } = defineProps({ hideHeader: Boolean });

// 根据当前 page 名称获取 sidebar 数据并构造相应的类别
// 使用 useRoute() 而非 window.location,以兼容服务端渲染(SSR)
